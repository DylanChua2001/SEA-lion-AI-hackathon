# app/subgraphs/immunisations.py
...
class ImmState(TypedDict):
    messages: Annotated[List, add_messages]
    tries: int

def _is_imm_url(u: Optional[str]) -> bool:
    if not isinstance(u, str): return False
    ul = u.lower()
    return ("immunisation" in ul) or ("immunization" in ul) or ("vaccination" in ul) or ("records" in ul)

def build_immunisations_subgraph(page: dict, tools: List):
    def node(state: ImmState) -> ImmState:
        ...
        if state["messages"] and isinstance(state["messages"][-1], ToolMessage):
            ...
            if name == "get_page_state":
                if _is_imm_url((data or {}).get("url")):
                    return {**state, "messages": [_ai("done", {"reason": "Arrived at Immunisation Records"})]}
                return {**state, "messages": [_ai("find", {"query": "Immunisation Records"})]}
            if name == "find":
                ...
                if t >= MAX_TRIES:
                    return {**state, "messages": [_ai("done", {"reason": "Immunisations link not found"})]}
                state["tries"] = t
